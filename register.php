...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$required_fields = ['fname', 'sex', 'age', 'date', 'address', 'phone_number', 'State', 'major', 'level', 'study-time', 'college'];
	...
	if (!empty($missing_fields)) {
		...
		$sql = "INSERT INTO Students (student_name, gender, age, birth_date, current_address, place_of_birth, skill, level, shift_id, phone_number, college_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

		$stmt = $conn->prepare($sql);
		if ($stmt) {
			$stmt->bind_param("ssisssssisi", $student_name, $gender, $age, $birth_date, $current_address, $place_of_birth, $skill, $level, $shift_id, $phone_number, $college_id);
			if ($stmt->execute()) {
				echo "<div class='row'>
...
<div class="container mt-5">
    ...
    <div class="row">
        <div class="col-md-6 form-group">
            <label for="address">អាស័យដ្ឋានបច្ចុប្បន្ន (បច្ចុប្បន្ន)</label><br />
